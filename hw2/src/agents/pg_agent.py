...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        discounted_sum = np.sum(
            [self.gamma**t * reward for t, reward in enumerate(rewards)]
        )
        return np.full(len(rewards), discounted_sum, dtype=np.float32)
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [self._discounted_return(reward_path) for reward_path in rewards]
        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            # TODO: use the helper function self._discounted_reward_to_go to calculate the Q-values
            q_values = [
                self._discounted_reward_to_go(reward_path) for reward_path in rewards
            ]
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    delta = (
                        rewards[i]
                        + self.gamma * values[i + 1] * non_terminal
                        - values[i]
                    )
                    advantages[i] = (
                        delta
                        + self.gamma * self.gae_lambda * advantages[i + 1] * non_terminal
                    )
